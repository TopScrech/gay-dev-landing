<script lang="ts">
  ...
  type Props = { src: string; alt: string; ratio?: string; eager?: boolean; cover?: boolean; class?: string }
  let { src, alt, ratio, eager = false, cover = false, class: className = '' }: Props = $props()

  let failed = $state(false)
  const dev = import.meta.env.DEV
</script>

<div class="asset {className}" class:cover class:blank={failed} style:aspect-ratio={ratio}>
  {#if !failed}
